		...
		public static void SRPT(ArrayList processes) {
			...
			while(hasremaining) {
				...
				for(int i = 0; i < processes.size(); i++) {
					...
					if(timer == ((Process)processes.get(i)).arrival) {
						if(processq.size() > 0) {
							if(((Process)processes.get(((Process)processq.get(0)).sequence-1)).burst > ((Process)processes.get(i)).burst) {
								// System.out.println("SECOND " + ((Process)processes.get(i)).sequence + " to the queue");
								// processq.add(1, ((Process)processes.get(i)));
								// System.out.println(((Process)processq.get(0)).burst + " > " + ((Process)processes.get(i)).burst);
								for(int j = 0; j < processq.size(); j++) {
									// if(((Process)processes.get(((Process)processq.get(j)).sequence-1)).burst >
									if(((Process)processq.get(j)).burst >
										((Process)processes.get(i)).burst) {
										processq.add(j, ((Process)processes.get(i)));
										System.out.println("Added " + ((Process)processes.get(i)).sequence + 
											" to index " + j);
										break;
									}
								}
								...
								// System.out.println(((Process)processq.get(0)).burst + " <= " + ((Process)processes.get(i)).burst);
								// System.out.println("FIRST " + ((Process)processes.get(i)).sequence + " to the queue");
								processq.add(0, ((Process)processes.get(i)));
							}
						} else {

							// System.out.println("LAST " + ((Process)processes.get(i)).sequence + " to the queue");
							processq.add(((Process)processes.get(i)));
						}
					} 
				}
				if(srptprocess.size() == 0) {
					srptprocess.add(new Process(
						((Process)processq.get(0)).sequence,
						((Process)processq.get(0)).arrival, 1,
						((Process)processq.get(0)).priority));
				} else {
					if(((Process)srptprocess.get(srptprocess.size()-1)).equals((Process)processq.get(0))) {
						// System.out.println(((Process)srptprocess.get(srptprocess.size()-1)).sequence + 
						// 	" is the same as " + ((Process)processq.get(0)).sequence);
						try {
							Thread.sleep(100);
						} catch(InterruptedException ex) {
							//do nothing
						}
						srptprocess.set((srptprocess.size()-1), new Process(
							((Process)processq.get(0)).sequence,
						...
						// System.out.println(((Process)srptprocess.get(srptprocess.size()-1)).sequence + 
						// 	" is NOT the same as " + ((Process)processq.get(0)).sequence);
						try {
							Thread.sleep(100);
						} catch(InterruptedException ex) {
							//do nothing
					...
					if((((Process)processq.get(0)).burst-1) == 0) {
						// System.out.println("REMOVE");
						processq.removeFirst();
					} else {
						// System.out.println("MINUS");
						processq.set(0, new Process(
							((Process)processq.get(0)).sequence,
							((Process)processq.get(0)).arrival,
							(((Process)processq.get(0)).burst-1),
							((Process)processq.get(0)).priority
					...
					if(processq.size() > 0) {
						// System.out.println(((Process)processq.get(0)).burst-1);
						processes.set((((Process)processq.get(0)).sequence-1), 
								new Process(((Process)processq.get(0)).sequence,
								((Process)processq.get(0)).arrival,
								((Process)processq.get(0)).burst-1,
								((Process)processq.get(0)).priority));	
					} else {
				...
				// System.out.println(((Process)processq.get(0)).sequence + " :: " + ((Process)processq.get(0)).burst);
				timer++;
			}
